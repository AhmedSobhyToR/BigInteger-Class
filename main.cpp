        ...
        if((num[0] == '-' || obj.num[0] == '-' ) && (num[0] != '-' && obj.num[0] != '-' )){
            ...
            if(num[0]=='-'){
                ...
                for(int i=num.length()-1;i>=0;i--){
                    n += (num[i]-48)*count;   // sum is existed in n (123)
                    count *=10;
                }
...
// Compares which number is less than the other one
// storing value in mo_str
            int diff;
            int length;
            string mo_str;
...
// Here when we sum 2 numbers, maybe one has more digits than the other
// diff is difference in digits
            int sum;
            int mo = 0;
            for(int i=1;i<=length;i++){
                ...
                // عشان بشوف اكبر من 10 ولا لا عشان لو اكبر اخد 4 مثلا واسيب ال1 اضيفه عل ىاللى بعديه
                if((digit1+digit2+mo)>=10){
                        mo = (digit2+digit1+mo) % 10;
// saving value in result
...
// Here when we sum 2 numbers, maybe one has more digits than the other
// For loop putting extra digits in bigger number
...
// removing first digit and replacing it by summing with bigger digits number
...
// Second If 2 negative numbers
// sum 2 negative numbers
// removing - then doing same steps as above
